...
class DtEntry(object):
    ...
    def __str__(self):
        sb = []
        sb.append("{key:>20} = {value:d}".format(key="dt_size", value=self.__dt_size))
        sb.append(
            "{key:>20} = {value:d}".format(key="dt_offset", value=self.__dt_offset)
        )
        sb.append("{key:>20} = {value:08x}".format(key="id", value=self.__id))
        sb.append("{key:>20} = {value:08x}".format(key="rev", value=self.__rev))
        if self.__version == 1:
...
def parse_dt_entry(global_args, arglist):
    ...
    parser.add_argument(
        "--id", type=str, dest="id", action="store", default=global_args.global_id
    )
    parser.add_argument(
        "--rev", type=str, dest="rev", action="store", default=global_args.global_rev
    )
    parser.add_argument(
        "--flags",
        type=str,
...
def parse_create_args(arg_list):
    ...
    parser.add_argument(
        "--dt_type", type=str, dest="dt_type", action="store", default="dtb"
    )
    parser.add_argument(
        "--page_size", type=int, dest="page_size", action="store", default=2048
    )
    parser.add_argument(
        "--version", type=int, dest="version", action="store", default=0
    )
    parser.add_argument("--id", type=str, dest="global_id", action="store", default="0")
    parser.add_argument(
        "--rev", type=str, dest="global_rev", action="store", default="0"
    )
    parser.add_argument(
        "--flags", type=str, dest="global_flags", action="store", default="0"
    )
    parser.add_argument(
        "--custom0", type=str, dest="global_custom0", action="store", default="0"
    )
    parser.add_argument(
        "--custom1", type=str, dest="global_custom1", action="store", default="0"
    )
    parser.add_argument(
        "--custom2", type=str, dest="global_custom2", action="store", default="0"
    )
    parser.add_argument(
        "--custom3", type=str, dest="global_custom3", action="store", default="0"
    )
...
def parse_dump_cmd_args(arglist):
    ...
    parser.add_argument("--dtb", "-b", nargs="?", type=str, dest="dtfilename")
...
def parse_config_create_cmd_args(arglist):
    ...
    parser.add_argument(
        "conf_file", nargs="?", type=argparse.FileType("r"), default=None
    )
    cwd = os.getcwd()
    parser.add_argument(
        "--dtb-dir", "-d", nargs="?", type=str, dest="dtbdir", default=cwd
    )
    return parser.parse_args(arglist)
...
def create_dtbo_image_from_config(fout, argv):
    ...
    global_args, dt_args = parse_config_file(
        args.conf_file, _DT_KEYS, _GLOBAL_KEY_TYPES
    )
...
def print_cfg_create_usage(progname):
    ...
    sb = []
    sb.append(
        "  " + progname + " cfg_create <image_file> <config_file> (<option>...)\n"
    )
    sb.append("    options:")
...
def print_usage(cmd, _):
    ...
    help_commands = (
        HelpCommand("dump", print_dump_usage),
...
def main():
    ...
    subparser = parser.add_subparsers(
        title="subcommand", description="Valid subcommands"
    )
